...
class UI {
    ...
    handleSourceChange(sourceId) {
        console.log('Source selection changed to:', sourceId);
        if (window.App && window.App.recorder) {
            window.App.recorder.setSelectedSource(sourceId);
        }
    }
    
    updateSourceList(sources, selectedId) {
        console.log('Updating source list with', sources.length, 'sources');
        ...
        console.log('Source list updated, selected:', selectedId);
    }
